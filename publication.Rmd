...
publications %>%
  ...
  mutate(nomes=str_replace(nomes,fixed("&"),fixed("\\&")),`journal-title.value`=str_replace(`journal-title.value`,fixed("&"),fixed("\\&")))%>% mutate(nomes=embolden_name(nomes)) %>%   detailed_entries(with=`title.title.value`, where = `journal-title.value`, when = `publication-date.year.value`, why=nomes, .protect =F)
```
